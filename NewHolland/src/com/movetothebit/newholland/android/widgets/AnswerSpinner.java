 ...
 */
public class AnswerSpinner extends Spinner implements OnMultiChoiceClickListener {
    String[] _items = null;
     ...
     * @param context
     */
    public AnswerSpinner(Context context) {
        super(context);
     ...
     * @param attrs
     */
    public AnswerSpinner(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    /**
     * Returns a array of strings, one for each selected item.
     * @return
     */
     ...
     * @return
     */
    public String[] getSelectedStringsArray() {
    	 List<String> selection = new LinkedList<String>();
        for (int i = 0; i < _items.length; ++i) {
            if (_selection[i]) {
                selection.add(_items[i]);
